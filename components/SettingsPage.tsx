  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="print:hidden">
        ...
        <p className="text-slate-400">Configure marcas, modelos, tipos de produtos e gerencie seus dados.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:hidden">
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm space-y-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-white">Tipos de Acessório</h3>
          </div>
          ...
          <form onSubmit={addAccessoryType} className="flex gap-2">
            <input 
              ...
              className="flex-1 bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder:text-slate-600"
              value={newTypeName}
          ...
          <div className="flex flex-wrap gap-2 max-h-[200px] overflow-y-auto pr-2">
            {state.accessoryTypes.map(type => (
              <div key={type} className="flex items-center gap-2 pl-3 pr-2 py-1.5 bg-slate-800 border border-slate-700 rounded-lg group">
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm space-y-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-white">Gerir Marcas</h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 max-h-[200px] overflow-y-auto pr-2">
            {state.brands.map(brand => (
              <div key={brand.id} className="flex items-center justify-between p-3 bg-slate-800 rounded-xl group">
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm space-y-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-white">Gerir Modelos</h3>
          </div>
          
          <form onSubmit={addModel} className="space-y-3">
            <select 
              className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-2 text-sm"
            ...
            <div className="flex flex-col gap-2">
              <input 
                ...
                className="w-full bg-slate-800 border border-slate-700 text-white rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder:text-slate-600"
                value={newModelName}
              ...
              <div className="flex gap-2">
                <div className="flex-1 relative">
                   ...
                   <input 
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-slate-800 border border-slate-700 text-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder:text-slate-600"
                    value={newModelColor}
          ...
          <div className="max-h-[300px] overflow-y-auto pr-2 space-y-2">
            {state.models.map(model => {
              ...
              return (
                <div key={model.id} className="flex items-center justify-between p-3 border border-slate-800 rounded-xl group">
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-indigo-400 uppercase">{brand?.name}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-slate-300">{model.name}</span>
                      <span className="text-xs text-slate-400 bg-slate-800 px-1.5 py-0.5 rounded-md font-medium">{model.color}</span>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm flex flex-col justify-between">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-white">Backup e Segurança</h3>
              </div>
              <p className="text-sm text-slate-400">Exporte os dados localmente ou envie por email para o desenvolvedor como forma de backup de segurança.</p>
            </div>
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button 
                onClick={exportData}
                className="py-3 bg-slate-800 text-slate-300 rounded-xl font-bold hover:bg-slate-700 transition-colors flex items-center justify-center gap-2"
              >
                <Database size={18} /> Exportar JSON
              </button>
              <button 
                onClick={backupViaEmail}
                className="py-3 bg-indigo-900/30 text-indigo-400 rounded-xl font-bold hover:bg-indigo-900/50 transition-colors flex items-center justify-center gap-2"
              >
          ...
          <div className="bg-indigo-600 p-6 rounded-2xl shadow-xl shadow-indigo-900/20 text-white flex flex-col justify-between">
             <div className="space-y-4">
              <div className="flex items-center gap-2">
                <ShieldCheck size={20} />
